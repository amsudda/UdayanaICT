...
export function AdminPacksPage() {
  ...
  const [vForm, setVForm] = useState<{ id: string; title: string; youtube: string; duration: string; description: string; tutes: { name: string; url: string }[] }>({ id: '', title: '', youtube: '', duration: '', description: '', tutes: [] });
  ...
  const saveVideo = async () => {
    ...
    if (vForm.id) {
      ...
      ({ error } = await supabase.from('pack_videos').insert({ ...payload, pack_id: videosPack.id, sort_order: nextOrder }));
    }
    if (error) {
