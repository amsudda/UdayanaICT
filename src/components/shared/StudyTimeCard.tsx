    ...
    <div className="rounded-3xl bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 p-4 sm:p-6 shadow-[0_4px_20px_rgba(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgba(0,0,0,0.2)]">
      <div className="flex items-center justify-between gap-3 mb-5">
        <div className="flex items-center gap-2">
          <ClockIcon className="w-5 h-5 text-apple-blue" />
          <h2 className="text-base sm:text-xl font-bold text-apple-text dark:text-apple-light">Study Time</h2>
        </div>
        <button onClick={openLog} className="flex items-center gap-1.5 h-9 px-3.5 rounded-xl bg-apple-blue text-white text-sm font-semibold hover:bg-blue-600 transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.6fr_1fr] gap-5 items-center">
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center justify-end h-full gap-2">
                  <div className="relative w-full max-w-[34px] flex-1 flex items-end">
                    <div
                      className={`w-full rounded-t-lg transition-all duration-500 ${isToday ? 'bg-apple-blue' : 'bg-blue-100 dark:bg-slate-700'}`}
                      style={{ height: `${Math.max(pct, h > 0 ? 6 : 2)}%` }}
                      title={`${h} h`}
                    />
                  </div>
                  <span className={`text-[11px] font-medium ${isToday ? 'text-apple-blue' : 'text-apple-subtext dark:text-slate-400'}`}>{DAY_LABELS[i]}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="space-y-2.5">
          {[
            { label: 'Daily average', value: avg, icon: TrendingUpIcon, tone: 'bg-blue-50 dark:bg-blue-500/10 text-apple-blue' },
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm bg-white dark:bg-slate-900 rounded-3xl shadow-2xl p-6">
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-2xl font-black text-apple-blue tabular-nums">{hours}</span>
            </div>
            <input
